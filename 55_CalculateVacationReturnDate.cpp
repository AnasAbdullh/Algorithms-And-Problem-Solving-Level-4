...
short CalculateVacationReturnDate(stDate & Date, stDate Vacation)
{
    short count = 0;
    while (IsDate1BeforeDate2(Date,Vacation)) {
...
int main()
{
    ...
    short Vacation = ReadVacationDays();

    stDate ddd;
    ddd.Day = Vacation;
    ddd.Month = Date.Month;
    ddd.Year = Date.Year;

    short count = CalculateVacationReturnDate(Date,ddd);

    Date =  DateAddDays(count + 3,Date);


    cout << "Return Date :" << DayShortName(Date.Day) << "," << " ";
